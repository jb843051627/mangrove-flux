...
func (l *Lab) EnqueueQuality(ctx context.Context, readingID string, done chan error) error {
	return l.queue.Submit(ctx, ingest.Job{ID: readingID, Ctx: context.Background(), Done: done, Run: func(jobCtx context.Context) error { _, err := l.EvaluateReading(jobCtx, readingID); return err }})
}
